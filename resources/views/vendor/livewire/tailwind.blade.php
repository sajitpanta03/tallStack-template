                            ...
                            @if (is_array($element))
                                @foreach ($element as $page => $url)
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                        ...
                                        @else
                                            <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold bg-zinc-100 border-zinc-200 border-y text-zinc-950 dark:text-zinc-50 dark:bg-zinc-950 hover:bg-zinc-200 dark:hover:bg-zinc-800 focus:bg-zinc-200 dark:focus:bg-zinc-800 dark:border-zinc-800" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
